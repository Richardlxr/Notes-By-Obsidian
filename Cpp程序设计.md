...
- **switch 贯穿（Fall-through）**：
    ...
    2. 找到 case 后看是否有 break，没有的话 default 是要贯穿执行的。
...
- **std::string (C++ 类)**：
    ...
    - 支持：length()、size()、at()（查越界）、substr()、find()。
...
- **字符串常量修改**：
    ...
    - char *p = "abc"：指向常量区，**不可修改**。
        
    - char arr[] = "abc"：数据在栈上，**可修改**。
...
- **常量成员函数**：
    ...
    - void f() const：内部不能修改任何非静态数据成员，只能调用其他常量成员函数。
...
- **对象切片（Slicing）**：
    ...
    - 按值传递派生类对象给基类时，派生类新增成员被舍弃，失去多态性。
        
- **虚析构函数必要性**：
    
    - 若类含虚函数，其析构函数应设为 virtual。
